...
#[derive(Debug, Serialize, JsonSchema)]
pub struct StartVmOutput {
    pub vms: Vec<StartedVmInfo>,
}

#[derive(Default)]
pub struct StartVmTool;

#[async_trait]
impl HyperVTool for StartVmTool {
    const NAME: &'static str = "hyperv_start_vm";
    const DESCRIPTION: &'static str = "Starts a virtual machine.";
    type Input = StartVmInput;
    type Output = StartVmOutput;
    ...
    async fn run(&self, ctx: &ToolContext, input: Self::Input) -> Result<Self::Output, ToolError> {
        ...
        let mut args = vec!["Start-VM".to_string()];
        ...
        Ok(StartVmOutput { vms: output })
    }
}

register_tool!(StartVmTool);
