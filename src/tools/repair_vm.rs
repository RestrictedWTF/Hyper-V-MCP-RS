...
#[derive(Debug, Serialize, JsonSchema)]
pub struct RepairVmOutput {
    pub vms: Vec<RepairVmInfo>,
}


#[derive(Default)]
pub struct RepairVmTool;

#[async_trait]
impl HyperVTool for RepairVmTool {
    const NAME: &'static str = "hyperv_repair_vm";
    const DESCRIPTION: &'static str = "Repairs one or more virtual machines.";
    type Input = RepairVmInput;
    type Output = RepairVmOutput;

    async fn run(&self, ctx: &ToolContext, input: Self::Input) -> Result<Self::Output, ToolError> {
        let mut args = vec!["Repair-VM".to_string()];
        ...
        Ok(RepairVmOutput { vms: output })
    }
}

register_tool!(RepairVmTool);
