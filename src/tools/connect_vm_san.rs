...
impl HyperVTool for ConnectVmSanTool {
    ...
    async fn run(&self, ctx: &ToolContext, input: Self::Input) -> Result<Self::Output, ToolError> {
        ...
        if input.world_wide_node_name.trim().is_empty() {
            return Err(ToolError::InvalidInput(
                "world_wide_node_name must not be empty".to_string(),
            ));
        }
        args.push(format!(
            "-WorldWideNodeName '{}'",
            escape_ps_string(&input.world_wide_node_name)
        ));
        if input.world_wide_port_name.trim().is_empty() {
            return Err(ToolError::InvalidInput(
                "world_wide_port_name must not be empty".to_string(),
            ));
        }
        args.push(format!(
            "-WorldWidePortName '{}'",
            escape_ps_string(&input.world_wide_port_name)
        ));
