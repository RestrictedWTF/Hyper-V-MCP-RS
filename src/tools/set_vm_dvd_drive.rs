use async_trait::async_trait;
use schemars::JsonSchema;
use serde::{Deserialize, Serialize};

use crate::ps_escape::escape_ps_string;
use crate::register_tool;
use crate::tool::{HyperVTool, ToolContext, ToolError};

#[derive(Debug, Deserialize, JsonSchema)]
pub struct SetVmDvdDriveInput {
    /// Name of the virtual machine whose DVD drive is to be configured.
    #[serde(rename = "vmName")]
    pub vm_name: String,
    /// Hyper-V host on which the virtual machine resides. Defaults to localhost.
    #[serde(default, rename = "computerName")]
    pub computer_name: Option<String>,
    /// IDE controller number of the DVD drive to configure.
    #[serde(default, rename = "controllerNumber")]
    pub controller_number: Option<i32>,
    /// Controller location of the DVD drive to configure.
    #[serde(default, rename = "controllerLocation")]
    pub controller_location: Option<i32>,
    /// Path to the ISO or virtual DVD file to insert. Pass an empty string to eject the disc.
    #[serde(default)]
    pub path: Option<String>,
    /// IDE controller number to move the DVD drive to.
    #[serde(default, rename = "toControllerNumber")]
    pub to_controller_number: Option<i32>,
    /// Controller location to move the DVD drive to.
    #[serde(default, rename = "toControllerLocation")]
    pub to_controller_location: Option<i32>,
    /// Allows the cmdlet to use a path that has not been verified.
    #[serde(default, rename = "allowUnverifiedPaths")]
    pub allow_unverified_paths: Option<bool>,
}

#[derive(Debug, Serialize, JsonSchema)]
pub struct DvdDriveInfo {
    #[serde(rename = "vmName")]
    pub vm_name: String,
    #[serde(rename = "computerName")]
    pub computer_name: String,
    #[serde(rename = "controllerNumber")]
    pub controller_number: i32,
    #[serde(rename = "controllerLocation")]
    pub controller_location: i32,
    pub path: String,
    #[serde(rename = "poolName")]
    pub pool_name: String,
    #[serde(rename = "isDeleted")]
    pub is_deleted: bool,
}

#[derive(Debug, Serialize, JsonSchema)]
pub struct SetVmDvdDriveOutput {
    pub drives: Vec<DvdDriveInfo>,
}

#[derive(Default)]
pub struct SetVmDvdDriveTool;

#[async_trait]
impl HyperVTool for SetVmDvdDriveTool {
    const NAME: &'static str = "hyperv_set_vm_dvd_drive";
    const DESCRIPTION: &'static str = "Configures a virtual DVD drive.";
    type Input = SetVmDvdDriveInput;
    type Output = SetVmDvdDriveOutput;

    async fn run(&self, ctx: &ToolContext, input: Self::Input) -> Result<Self::Output, ToolError> {
        if input.vm_name.trim().is_empty() {
            return Err(ToolError::InvalidInput(
                "VM name must not be empty".to_string(),
            ));
        }

        let mut args = vec![format!(
            "Set-VMDvdDrive -VMName '{}'",
            escape_ps_string(&input.vm_name)
        )];

        if let Some(computer) = &input.computer_name {
            args.push(format!("-ComputerName '{}'", escape_ps_string(computer)));
        }
        if let Some(number) = input.controller_number {
            args.push(format!("-ControllerNumber {}", number));
        }
        if let Some(location) = input.controller_location {
            args.push(format!("-ControllerLocation {}", location));
        }
        if let Some(path) = &input.path {
            if path.trim().is_empty() {
                args.push("-Path $null".to_string());
            } else {
                args.push(format!("-Path '{}'", escape_ps_string(path)));
            }
        }
        if let Some(number) = input.to_controller_number {
            args.push(format!("-ToControllerNumber {}", number));
        }
        if let Some(location) = input.to_controller_location {
            args.push(format!("-ToControllerLocation {}", location));
        }
        if input.allow_unverified_paths.unwrap_or(false) {
            args.push("-AllowUnverifiedPaths".to_string());
        }


        let ps = format!(
            "{} | Select-Object \
             VMName, ComputerName, ControllerNumber, ControllerLocation, Path, PoolName, IsDeleted \
             | ConvertTo-Json -Compress -Depth 3",
            args.join(" ")
        );

        let json = ctx
            .sidecar
            .execute(&ps, ctx.timeout)
            .await
            .map_err(|e| ToolError::Sidecar(e.to_string()))?;

        let json_sanitized = if json.trim().is_empty() { "[]" } else { &json };
        let raw: serde_json::Value = serde_json::from_str(json_sanitized)?;

        let drives = match raw {
            serde_json::Value::Array(arr) => arr,
            other => vec![other],
        };

        let mut output = Vec::with_capacity(drives.len());
        for drive in drives {
            output.push(DvdDriveInfo {
                vm_name: drive["VMName"].as_str().unwrap_or_default().to_string(),
                computer_name: drive["ComputerName"]
                    .as_str()
                    .unwrap_or_default()
                    .to_string(),
                controller_number: drive["ControllerNumber"].as_i64().unwrap_or_default() as i32,
                controller_location: drive["ControllerLocation"].as_i64().unwrap_or_default()
                    as i32,
                path: drive["Path"].as_str().unwrap_or_default().to_string(),
                pool_name: drive["PoolName"].as_str().unwrap_or_default().to_string(),
                is_deleted: drive["IsDeleted"].as_bool().unwrap_or_default(),
            });
        }

        Ok(SetVmDvdDriveOutput { drives: output })
    }
}

register_tool!(SetVmDvdDriveTool);
