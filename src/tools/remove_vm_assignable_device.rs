...
impl HyperVTool for RemoveVmAssignableDeviceTool {
    ...
    const DESCRIPTION: &'static str = "Removes information about the assignable devices from a specific virtual machine.";
    ...
    async fn run(&self, ctx: &ToolContext, input: Self::Input) -> Result<Self::Output, ToolError> {
        ...
        if let Some(location_path) = &input.location_path {
            if location_path.trim().is_empty() {
                return Err(ToolError::InvalidInput("location_path must not be empty when provided".to_string()));
            }
            args.push(format!("-LocationPath '{}'", escape_ps_string(location_path)));
        }
