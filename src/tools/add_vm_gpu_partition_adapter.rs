...
pub struct VmGpuPartitionAdapterAddInfo {
    pub name: String,
    pub id: String,
    #[serde(rename = "vmName")]
    pub vm_name: String,
...
pub struct AddVmGpuPartitionAdapterInput {
...
#[derive(Debug, Serialize, JsonSchema)]
pub struct AddVmGpuPartitionAdapterOutput {
    pub adapters: Vec<VmGpuPartitionAdapterAddInfo>,
}

#[derive(Default)]
pub struct AddVmGpuPartitionAdapterTool;

#[async_trait]
impl HyperVTool for AddVmGpuPartitionAdapterTool {
    const NAME: &'static str = "hyperv_add_vm_gpu_partition_adapter";
    const DESCRIPTION: &'static str = "Adds a GPU partition adapter to a virtual machine.";
    type Input = AddVmGpuPartitionAdapterInput;
    type Output = AddVmGpuPartitionAdapterOutput;

    async fn run(&self, ctx: &ToolContext, input: Self::Input) -> Result<Self::Output, ToolError> {
        let mut args = vec!["Add-VMGpuPartitionAdapter".to_string()];
        ...
        for item in items {
            output.push(VmGpuPartitionAdapterAddInfo {
        ...
        Ok(AddVmGpuPartitionAdapterOutput { adapters: output })
    }
}

register_tool!(AddVmGpuPartitionAdapterTool);
