    ...
    async fn run(&self, ctx: &ToolContext, input: Self::Input) -> Result<Self::Output, ToolError> {
        ...
        if let Some(network_adapter_name) = &input.network_adapter_name {
            if network_adapter_name.trim().is_empty() {
                return Err(ToolError::InvalidInput("network_adapter_name must not be empty when provided".to_string()));
            }
            args.push(format!("-NetworkAdapterName '{}'", escape_ps_string(network_adapter_name)));
        }
