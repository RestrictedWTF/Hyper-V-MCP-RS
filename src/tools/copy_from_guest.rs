...
pub struct CopyFromGuestOutput {
    pub success: bool,
...
#[derive(Default)]
pub struct CopyFromGuestTool;

#[async_trait]
impl HyperVTool for CopyFromGuestTool {
    const NAME: &'static str = "hyperv_copy_from_guest";
    const DESCRIPTION: &'static str =
        "Copies a file from a guest VM out to the host filesystem using PowerShell Direct.";
    type Input = CopyFromGuestInput;
    type Output = CopyFromGuestOutput;
...
try {{
    ...
    Copy-Item -Path '{}' -Destination '{}' -FromSession $session -Force -ErrorAction Stop;
...
"#,
        ...
        let output: CopyFromGuestOutput = serde_json::from_str(&json)?;
        Ok(output)
    }
}

register_tool!(CopyFromGuestTool);
